...
// ################################################# TFT ###################################################
/* процедуры упраления TFT ------------------------------------------------------------ */
#pragma optimize=none
static inline void ST7789_SendCmd(U16 command)
{
  RS_TFT_LOW;
  GPIOE->ODR = command;
  WR_TFT_LOW;
...
#pragma optimize=none
static inline void ST7789_SendData(U16 data)
{
  RS_TFT_HIGH;
  GPIOE->ODR = data;
  WR_TFT_LOW;
...
#pragma optimize=none
static inline U16 ST7789_ReadData(void)
{
  RS_TFT_HIGH;
  GPIOE->MODER = 0;
  RD_TFT_LOW;  
...
#pragma optimize=none
static inline void write_control_TFT(U8 adr, U16 data)
{  
  // write to IR
  RS_TFT_LOW;
  GPIOE->ODR = adr;
...
#pragma optimize=none
static inline U16 read_control_TFT(U8 adr)
{ 
  // write to IR adr
  RS_TFT_LOW;
  GPIOE->ODR = adr;
...
{
  switch( system_flags.tftdriver )
  {
  case TFT_DRIVER_ILI9325:  
      // write to IR
      RS_TFT_LOW;
      GPIOE->ODR = 0x22;  
      WR_TFT_LOW;
  ...
  case TFT_DRIVER_ST7789:       
      ST7789_SendCmd( 0x2C); // write
      RS_TFT_HIGH;
      for(U32 i = 0; i < PixNum;  i++)
      {
        // write data      
...
{ 
  div_t       n = div(PixNum, 256);
  
  start_write_GRAM_TFT();
  open_SDCard(SDaddress);
    
  for(U32 i = 0; i < n.quot; i++)
  {    
...
{   
  ...
  case TFT_DRIVER_ST7789: 
      ST7789_SendCmd( 0x2C); // write command
      RS_TFT_HIGH;
      break;
  }  
}
...
{
  for(U32 i = 0; i < NumWrites;  i++)
  {
   // write data  
    GPIOE->ODR = sourceMemory[i];
    WR_TFT_LOW;
    WR_TFT_HIGH;
  }
}
...
void TFT_SetArea(Int16U sX, Int16U eX, Int16U sY, Int16U eY)
{
  ...
  case TFT_DRIVER_ILI9325:   
    ...
    write_control_TFT(0x20, RESOLUTION_Y - sY - 1);
    write_control_TFT(0x21, sX);
  break;
  
  case TFT_DRIVER_ST7789:  
    
    ST7789_SendCmd( 0x2A);
    ST7789_SendData(sX >> 8);
    ST7789_SendData(sX);
    ST7789_SendData(eX >> 8);
    ST7789_SendData(eX);
    ST7789_SendCmd( 0x2B);
    ST7789_SendData(sY >> 8);
    ST7789_SendData(sY);
    ST7789_SendData(eY >> 8);
    ST7789_SendData(eY);
    break;
  }  
}
...
#pragma optimize=none
void ST7789_StopReadFM(void)
{       
  GPIOE->MODER = 0x55555555;
}

#pragma optimize=none
U16 ST7789_ReadFM(void)
{
  RD_TFT_LOW;
  __no_operation();
  __no_operation();
...
#define PixelNum            RESOLUTION_Y * RESOLUTION_X
  ...
  switch( system_flags.tftdriver )
  {
    ...
    bytePerPixel = 2;  
    sdwritenum   = PixelNum * bytePerPixel / SD_sectorSize;
    
    TFT_SetArea(0, RESOLUTION_X - 1, 0, RESOLUTION_Y - 1);
    start_read_GRAM_TFT();
    read_GRAM_TFT();  // dummy read data
  
    for(U32 i = 0; i < sdwritenum; i++, SDaddress += SD_sectorSize)
    { 
      for(U32 j = 0; j < SD_sectorSize / 2; j++) ibuf[j] = read_GRAM_TFT();
      errors_flags.sd_fail |= write_SDCard( (pU8)buf, SDaddress, SD_sectorSize);
    } 
    stop_read_GRAM_TFT();
    break;
    ...
    case TFT_DRIVER_ST7789:
      ...
      ST7789_SendCmd(0x3A); ST7789_SendData(0x06);  // set 18 bit RGB - manual req.
      TFT_SetArea(0, RESOLUTION_X - 1, 0, RESOLUTION_Y - 1);
      ST7789_SendCmd(0x2E); // read command
      ST7789_StartReadFM();
      ST7789_ReadFM();      // dummy read data
      
      for(U32 i = 0; i < sdwritenum; i++, SDaddress += SD_sectorSize)
      { 
        for(U32 j = 0; j < SD_sectorSize / 2; j++) ibuf[j] = ST7789_ReadFM();
        errors_flags.sd_fail |= write_SDCard( (pU8)buf, SDaddress, SD_sectorSize);
      }
      ST7789_StopReadFM();
      
      ST7789_SendCmd(0x3A); ST7789_SendData(0x05);  // 16 bit RGB
      break;
  }    
}
...
void GUI_LogoScreen()
{
  ...
  GUI_DrawImageInRectCenterSwapColor(EKSIS_LOGO_320x240, SCREEN, TEMPLATE_COLOR, WHITE);
  OS_Delay(4000);
}
...
void GUI_UpdateMeasuresAll()
{
  ...
  for (int i = 0; i < channels_count; i++)
  {
    ...
    if ((first != -1) && (second != -1))
    {
      ...
      TFT_FillRect_(*LParams[i], back_color);
      actual_text_rect = TFT_WriteStringInRectLeft(display, color, back_color, *LParams[i], 0, 0, &Tahoma43);
      
      GetParamUnits(i, first, display);
      TFT_WriteString(display, BLACK, actual_text_rect.color, actual_text_rect.eX+6, actual_text_rect.sY, &Tahoma19B);
      
      if (DeviceName[0]=='М')
      {
        GetParamType(i, first, display);
        TFT_WriteString(display, BLACK, actual_text_rect.color, actual_text_rect.eX+6, actual_text_rect.sY+19, &Tahoma19B);
      }
          
      GetMeasureStringNoUnits(i, second, display, &color);
      TFT_FillRect_(*RParams[i], back_color);
      actual_text_rect = TFT_WriteStringInRectLeft(display, color, back_color, *RParams[i], 0, 0, &Tahoma43);
      
      GetParamUnits(i, second, display);
      TFT_WriteString(display, BLACK, actual_text_rect.color, actual_text_rect.eX+6, actual_text_rect.sY, &Tahoma19B);
      
      if (DeviceName[0]=='М')
      {
        GetParamType(i, second, display);
        TFT_WriteString(display, BLACK, actual_text_rect.color, actual_text_rect.eX+6, actual_text_rect.sY+19, &Tahoma19B);
      }
    }
    else if (first != -1)
    {
      GetMeasureStringNoUnits(i, first, display, &color);
      TFT_FillRect_(*Params[i], back_color);
      actual_text_rect = TFT_WriteStringInRectCenter(display, color, back_color, *Params[i], 0, 0, &Tahoma43);
...
void GUI_DrawChannelSettingsRootScreen()
{
  ...
  if (OxygenCalculationPossible(CurrentDisplay))
  {
    TFT_WriteStringInRectCenter("Параметры пересчёта кислорода", DARKGREEN, BACKGROUND_COLOR, UPPER, 0, 0, &Tahoma19B);
    TFT_UnderlineRect_Offset(UPPER, BLACK, 1, 40, 0);
  }
  ...
  if (PressureCalculationPossible(CurrentDisplay))
  {
    ...
    TFT_WriteStringInRectCenter("Параметры датчика давления", DARKGREEN, BACKGROUND_COLOR, LOWER, 0, 0, &Tahoma19B);
//    TFT_UnderlineRect_Offset(UPPER, BLACK, 1, 40, 0);
...
void GUI_TouchChannelSettingsRootScreen()
{
  ...
  if (TFT_CoordsInRect(X, Y, UPPER))
  {
    ...
    {
      set_sound(S_SOUND_20);
      GUI_SetGUI(GUI_DrawChannelSettingsCalculateOxygenScreen, GUI_UpdateChannelSettingsCalculateOxygenScreen, GUI_TouchChannelSettingsCalculateOxygenScreen);
      return;
    }
  }
...
void GUI_DrawChannelSettingsCalculateOxygenScreen()
{
  ...
  GUI_UpdateChannelSettingsCalculateOxygenScreen();
}

void GUI_UpdateChannelSettingsCalculateOxygenScreen()
{
  char display[25];
  Int8U index = GetOxygenParamIndex(CurrentDisplay);
  Int8U measure_attribute = DisplayConfig[CurrentDisplay].DisplayParam[index];
  Int8U calc_config_index = measure_attribute - CALC_BASE;
  int index0 = CalcConfig[calc_config_index].InputConstantParamNum[0]%12;
...
void GUI_TouchChannelSettingsCalculateOxygenScreen()
{
  ...
  if (TFT_CoordsInRect(X, Y, RIGHTHALF_UPPER))
  {
    ...
    else
    {
      ...
      //GUI_KeyboardFloat(&DisplayConfig[CurrentDisplay].Constant[index*2], "Настройки", "Значение, атм", true, 0, 1000, decimal);
      GUI_KeyboardFloat(&DisplayConfig[CurrentDisplay].Constant[index0], "Настройки", "Значение, атм", true, 0, 1000, decimal);
      GUI_DrawChannelSettingsCalculateOxygenScreen();
      return;
    }
  }
}
  ...
  if (BackToAllChannels) // пользователь зашёл из общего экрана
  {
    ...
    if ((first != -1) && (second != -1))
    {
      if (CurrentParam == first)
      {
        ...
        TFT_FillRect_(*LParams[CurrentDisplay], back_color);
        actual_text_rect = TFT_WriteStringInRectLeft(display, color, back_color, *LParams[CurrentDisplay], 0, 0, &Tahoma43);
        
        GetParamUnits(CurrentDisplay, first, display);
        TFT_WriteString(display, BLACK, actual_text_rect.color, actual_text_rect.eX+6, actual_text_rect.sY, &Tahoma19B);
      }
      if (CurrentParam == second)
      {
        GetMeasureStringNoUnits(CurrentDisplay, second, display, &color);
        TFT_FillRect_(*RParams[CurrentDisplay], back_color);
        actual_text_rect = TFT_WriteStringInRectLeft(display, color, back_color, *RParams[CurrentDisplay], 0, 0, &Tahoma43);
        
        GetParamUnits(CurrentDisplay, second, display);
        TFT_WriteString(display, BLACK, actual_text_rect.color, actual_text_rect.eX+6, actual_text_rect.sY, &Tahoma19B);
      }
    }
    else if (first != -1)
    {
      GetMeasureStringNoUnits(CurrentDisplay, first, display, &color);
      TFT_FillRect_(*Params[CurrentDisplay], back_color);
      actual_text_rect = TFT_WriteStringInRectCenter(display, color, back_color, *Params[CurrentDisplay], 0, 0, &Tahoma43);
...
void GUI_UpdateRelayConfigurationCommonScreen()
{
  ...
  {
    TFT_GrayoutRect_(CHANNEL0, BACKGROUND_COLOR);
    TFT_FillRect_(RIGHT_TWOTHIRDS_UPPER, BACKGROUND_COLOR); //!!!
    TFT_WriteStringInRectLeftInTwoLines("Управляемый", "параметр", BLACK, BACKGROUND_COLOR, LEFT_TWOTHIRDS_UPPER, 3, 0, &Tahoma19B);
    //TFT_FillRect_(RIGHT_THIRD_UPPER, BACKGROUND_COLOR);
    GetControlChannelInputParamString(OutConfig[CurrentControl].Input, display);
    TFT_WriteStringInRectCenter(display, DARKGREEN, BACKGROUND_COLOR, RIGHT_THIRD_UPPER, 0, 0, &Tahoma19B);
...
void GUI_TouchRelayConfigurationCommonScreen()
{
  ...
  if (button == SWITCH_BUTTON)
  {
    ...
    errors_flags.sd_fail |= read_SDCard ( (pInt8U)Program, CONFIG_ROM_ADR(Program) + SD_CONFIG_ADR, 512*sizeof(Program_t) );
    OS_Unuse(&SemaRAM);
...
void GUI_TouchSettingsDateScreen()
{
  ...
  if (TFT_CoordsInRect(X, Y, RIGHT_TWOTHIRDS_UPPERMOST))
  {
    set_sound(S_SOUND_20);
    ParseDate( TempTime, &s, &mi, &h, &d, &m, &y);
    if (GUI_KeyboardInt( &y, "Настройки", "Год", true, 2021, 2099))
...
{   
  ...
  ST7789_SendCmd(0xDA);   // Read Display ID   
  ST7789_ReadData();
  U8 mnfID  = ST7789_ReadData();

  CS_TFT_HIGH;
  
  if( mnfID != 0x85 ) return 1;
  
  CS_TFT_LOW; 
  
  system_flags.tftdriver = TFT_DRIVER_ST7789;
  ST7789_SendCmd(0x01);   // SWRESET
  OS_Delay(120);          // 120 ms после сброса в SleepMode
  ...
  ST7789_SendCmd(0x3A);   // RGB format
  ST7789_SendData(0x05);  // RGB565(16bit) 0x05, RGB666(18bit) 0x06

  ST7789_SendCmd(0x36);   // MADCTL
  ST7789_SendData(0xa0);  // 0x08 B-G-R, 0x00 R-G-B
  
/*   ST7789_SendCmd(0xbb);   // VCOM Setting
   ST7789_SendData(0x2b);
   
   ST7789_SendCmd(0xc2);
   ST7789_SendData(0x01);

   ST7789_SendCmd(0xc3);   // VRH Set
   ST7789_SendData(0x0b);
   
   ST7789_SendCmd(0xc4);   // VDV Setting
   ST7789_SendData(0x20);
   
   ST7789_SendCmd(0xc6);   // Frame Rate Control in Normal Mode
   ST7789_SendData(0x0f);
   
   ST7789_SendCmd(0xd0);   // Power Control 1
   ST7789_SendData(0xa4);   // 0xA4 Required
   ST7789_SendData(0xa2);   // AVDD[1:0], AVCL[1:0], VDS[1:0]
   
   ST7789_SendCmd(0xe0);   // Positive Voltage Gamma Control
   ...
   ST7789_SendCmd(0xe1);   // Negative Voltage Gamma Control
  ...
  TFT_FillScreen(BLACK);  // clear GRAM

  ST7789_SendCmd(0x29);   // DISP ON 
    
  return NO_ERR;
} 

U8 ILI9325_Init(void)
{   
/* сброс */
  CS_TFT_HIGH;
  RES_TFT_LOW;
...
/* Start Initial Sequence --------------------------------------------------*/  
  ...
  system_flags.tftdriver = TFT_DRIVER_ILI9325;
...
U8 TFT_init(void)
{     
  if( ILI9325_Init() == NO_ERR ) return NO_ERR;
  if( ST7789_Init()  == NO_ERR ) return NO_ERR;
...
//  GUI_SetGUI(GUI_DrawControlsScreen, GUI_UpdateControlsScreen, GUI_TouchControlsScreen);
  ...
  for(;;)
  {
    ...
    if ((update_counter++ % 20 == 0) && update) 
    {
      switch( system_flags.tftdriver )
      {
        case TFT_DRIVER_ILI9325:  tft_trouble = (read_control_TFT(0x01) != 0x0100);
                                  break;
                                  
        case TFT_DRIVER_ST7789:   ST7789_SendCmd(0x0B); ST7789_ReadData();                                  
                                  tft_trouble = ((ST7789_ReadData() & 0xf0) != 0xa0);
                                  break;
      }
