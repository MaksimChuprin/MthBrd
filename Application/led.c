...
static volatile U8 led_start = 1;//блокируем при старте программы

//Вывод на экран и работа с клавиатурой
void led_keyboard ()
{
  static Int8U          key_flag;
  static Int8U          FlashFlag;
  static Int16U         timer1s;
  ...
  {
     if( DisplPosition )        DisplPosition--;
     else                       DisplPosition = 9;
  }  
}
...
Int8S value_to_str (int display_channel_index, int dispay_param_index, char *str/*, int *color*/)
{
  ...
  {
    if ((MeasureAttribute[DisplayConfig[current_channel_index].DisplayParam[dispay_param_index]].ParamType==3)&&//Если это давление в ПКГ О2 и датчик давления не подключен - ошибку не выводим
             (device_type==PKG_O2) && ErrorArray[param_index]==CONNECT_ERR) sprintf (str, "    ");
    else if ((temp_ind_width==4)||(device_type==MAG)) sprintf (str, "E-%02X", ErrorArray[param_index]);
...
const char* const menu_time[]={"Year", "NNon", "Day ", "Hour", "NNin", "Sec "};
...
const char* const menu_p[]={"ConS", "par "};
...
void reset_stat ()
{
  Int8S reset=0;
  FlashMaskDisplay=15<<6;
  j=show_list(menu_yesno, sizeof (menu_yesno)/sizeof (char *), &reset, menu_pos);
  FlashMaskDisplay=0;
  if((j == ABORT) || (j == NO_KEY)) return;
  if (reset==0) return;
  OS_Use(&SemaRAM);
...
{  
  ...
  ParseDate( Time, &second, &minute, &hour, &day, &month, &year);
  for (;;)
  {
    FlashMaskDisplay=15<<menu_pos;
    print_parametr ("    ", menu_pos);
    j=show_list(menu_time, sizeof (menu_time)/sizeof (char *), &t, 2);
    if ((j == ABORT) || (j == NO_KEY))
    { 
      FlashMaskDisplay=0;
      OS_Use(&SemaRAM);
        Time = EncodeDate ( second, minute, hour, day, month, year);
        RTC_SetCounter (Time);
        errors_flags.timeinvalid = 0;
        set_bkp_reg( ERR_BASE, get_bkp_reg(ERR_BASE) & ~(1L<<1));
      OS_Unuse(&SemaRAM);
      if(ConfigWord & TCP_PRESENT) Initialize_TCPIP_Config(0);
      break; 
    }
    switch (t)
    {
    case 0:
      year=set_int ((Int16S) year, 2019, 2099, menu_pos);
      break;
    case 1:
      month=set_int ((Int16S) month, 1, 12, menu_pos);
      break;
    case 2:
      day=set_int ((Int16S) day, 1, 31, menu_pos);
      break;
    case 3:
      hour=set_int ((Int16S) hour, 0, 23, menu_pos);
      break;
    case 4:
      minute=set_int ((Int16S) minute, 0, 59, menu_pos);
      break;
    case 5:
      second=set_int ((Int16S) second, 0, 59, menu_pos);
      break;      
    }    
  }
}
...
//Без датчиков давления (ИВТМ, ПКУ, ПКГ CO и МАГ)
const char* const menu_main_ivtm[]=    {"Adr ", "rS  ", "beep", "trs ", "defL", "Per ", "date", "res ", "iP  ", "ser ", "dis "};
void (*menu_main_func_ivtm[]) ()={rs485_address, rs485_speed, beeps, tresholds, defaults, stat_period, time_settings, reset_stat, net_settings, webserver_acivity, webserver_stat};

//Настраиваются два датчика давления на один канал (ИВГ)
const char* const menu_main_ivg[]={"Adr ", "rS  ", "beep", "trs ", "defL", "Per ", "cons", "sens", "date", "res ", "iP  ", "ser ", "dis "};
void (*menu_main_ivg_func[]) ()={rs485_address, rs485_speed, beeps, tresholds, defaults, stat_period, select_pressure_sensors, pressure_sensor, time_settings, reset_stat, net_settings, webserver_acivity, webserver_stat};

//Настраивается один датчик давления на один канал (ПКГ О2)
const char* const menu_main_pkg[]={"Adr ", "rS  ", "beep", "trs ", "defL", "Per ", "cons", "sens", "date", "res ", "iP  ", "ser ", "dis "};
void (*menu_main_pkg_func[]) ()={rs485_address, rs485_speed, beeps, tresholds, defaults, stat_period, select_concentration_sensors, pressure_sensor, time_settings, reset_stat, net_settings, webserver_acivity, webserver_stat};

//Для ТТМ ввод размера воздуховода
const char* const menu_main_ttm[]=    {"Adr ", "rS  ", "beep", "trs ", "defL", "Per ", "cons", "date", "res ", "iP  ", "ser ", "dis "};
void (*menu_main_func_ttm[]) ()={rs485_address, rs485_speed, beeps, tresholds, defaults, stat_period, airduct, time_settings, reset_stat, net_settings, webserver_acivity, webserver_stat};
...
void ProgManager1 ()
{
  ...
  for (;;)
  {
    ...
    else if ((device_type==IVTM)||(device_type==PKG_CO)||(device_type==MAG)||(device_type==PKU))
    {
...
void led_timer_task ()
{
  ...
  else if ((device_type==PKG_CO)&&(strstr ((char const *)DeviceName, "16")!=0)) DisplayBuffer[18]=1<<(current_param_index+1);//ПКГ 16 реле сдвинуты светодиоды
  else if ((device_type==TTM)&&(strstr ((char const *)DeviceName, "16")!=0)) DisplayBuffer[18]=1<<(current_param_index-start_param_index+1);//ТТМ 16 сдвинуты светодиоды
  else if ((GetDisplayChannelParamsCount(current_channel_index)==1)&&(device_type!=MAG)) DisplayBuffer[18]=0;
  else 
  {
    if (device_type==MAG)
    {
    ...
    else if ((device_type==PKG_O2)&&(current_param_index==2)&&(strstr ((char const *)DeviceName, "С-2А")!=0)) DisplayBuffer[18]=1<<2;
    else if ((device_type==PKG_CO)&&(current_param_index==1)&&(strstr ((char const *)DeviceName, "С-2А")!=0)) DisplayBuffer[18]=1<<2;
    else if ((device_type==TTM)&&(current_param_index==2)&&(strstr ((char const *)DeviceName, "С-2А")!=0)) DisplayBuffer[18]=1<<2;
    else if ((device_type==PKU)&&(current_param_index==1)) DisplayBuffer[18]=1<<2;
  ...
  if ((device_type==MAG) 
      //&& (GetDisplayChannelsCount()>1))
      &&(GetOutChannelsCount()>2)) DisplayBuffer[16]|=(1<<(current_channel_index+4));
...
void led_init ()
{
  ...
  DisplayBuffer[16] = 0;
  if (control_leds_exist)       DisplayBuffer[17] = 1<<current_out_channel;  
  else                          DisplayBuffer[17] = 0;
  DisplayBuffer[18] = 0;
  
  OS_CREATEMB   ( &key_mb, MAIL_SIZE, MAIL_COUNT, mail_storage);//Коды нажатых кнопок
  OS_CREATETIMER( &LEDTimer, led_keyboard, 2);//обслуживание клавиатуры и индикаторов
